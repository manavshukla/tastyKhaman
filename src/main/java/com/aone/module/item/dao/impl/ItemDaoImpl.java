...
public class ItemDaoImpl extends AbstractBaseMasterRepository<Item, Long> implements IItemDao {
	...
	public int updateItemStock(Item item) {
		...
		queryBuilder.append(" SET stock = stock + :stock, "); 
		queryBuilder.append(" lastUpdatedDateTime = :lastUpdatedDateTime");
		queryBuilder.append(" where id = :id");
		Query query = getEntityManager().createQuery(queryBuilder.toString());
		query.setParameter("id", item.getId());
		query.setParameter("stock", item.getStock());
		query.setParameter("lastUpdatedDateTime", LocalDateTime.now());
...
//		}
	...
	@Override
	protected <R> TypedQuery<R> getQueryObject(SearchData<Item> searchData, boolean isGetAll, boolean isCountQuery, Class<R> clazz){
		StringBuilder queryBuilder = new StringBuilder(50);
		...
		if(!isGetAll) {
			...
			if(isPublicModule()) {
				...
				if(searchData != null) {
					...
					if(item != null ) {
						if(item.getName() != null && !item.getName().isEmpty()) {
							queryBuilder.append(" e.name like :itemName AND ");
						}
					}
					queryBuilder.append(" e.deleted = :deleted ");
				}
